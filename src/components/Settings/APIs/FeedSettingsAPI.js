export const fetchFeedSettings = async (setSnackbarInfo) => {
  try {
    const response = await fetch('https://www.threadit.tech/api/v1/users/me/settings/feedSettings');
    if (response.ok) {
...
export const updateFeedSettings = async (newSettings, setSnackbarInfo) => {
  try {
    const response = await fetch('https://www.threadit.tech/api/v1/users/me/settings/updateFeedSettings', {
      method: 'POST',
      headers: {
