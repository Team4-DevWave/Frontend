...
const OldNotification = ({ setNotificationCount }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://www.threadit.tech/api/v1/notifications", config)
      .then((response) => {
  ...
  const markAsRead = (index) => {
    ...
    axios
      .patch(
        `https://www.threadit.tech/api/v1/notifications/hide/${notificationId}`,
        {},
        config
      )
      .then((response) => {
    ...
    axios
      .patch(
        `https://www.threadit.tech/api/v1/notifications/read/${notificationId}`,
        {},
        config
      )
      .then((response) => {
  ...
  const handleNotificationClick = (notification) => {
    ...
    switch (notification.type) {
      case "comment":
        navigate(
          `/comments/${notification.contentID.id}/${notification.contentID.title}`
        );
        break;
